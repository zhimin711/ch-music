 ...
 */
package com.ch.music.fragments.folder
...
import com.ch.appthemehelper.ThemeStore.Companion.accentColor
import com.ch.appthemehelper.common.ATHToolbarActivity
import com.ch.appthemehelper.util.ToolbarContentTintHelper
import com.ch.music.R
import com.ch.music.adapter.SongFileAdapter
import com.ch.music.adapter.Storage
import com.ch.music.adapter.StorageAdapter
import com.ch.music.adapter.StorageClickListener
import com.ch.music.databinding.FragmentFolderBinding
...
import com.ch.music.helper.ScanMusicBottomSheet

import com.ch.music.helper.menu.SongMenuHelper
import com.ch.music.helper.menu.SongsMenuHelper
import com.ch.music.interfaces.ICallbacks
import com.ch.music.interfaces.IMainActivityFragmentCallbacks
import com.ch.music.interfaces.IScrollHelper
import com.ch.music.misc.FolderMediaScannerCompletionListener

import com.ch.music.misc.WrappedAsyncTaskLoader
import com.ch.music.model.Song
import com.ch.music.providers.BlacklistStore
import com.ch.music.util.FileUtil
import com.ch.music.util.getExternalStorageDirectory
import com.ch.music.util.getExternalStoragePublicDirectory
import com.ch.music.helper.MusicPlayerRemote
import com.ch.music.util.PreferenceUtil
import com.ch.music.util.ThemedFastScroller
import com.ch.music.views.BreadCrumbLayout
import com.ch.music.views.BreadCrumbLayout.Crumb
import com.ch.music.views.BreadCrumbLayout.SelectionCallback
...
class FoldersFragment : AbsMainActivityFragment(R.layout.fragment_folder),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            val restoreDir = if (PreferenceUtil.saveLastDirectory) {
                PreferenceUtil.lastDirectory;
            } else {
            ...
            popupMenu.setOnMenuItemClickListener { item: MenuItem ->
                when (val itemId = item.itemId) {
                    ...
                    R.id.action_set_as_start_directory -> {
                        PreferenceUtil.startDirectory = file
                        showToast(
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                listSongs(
                    ...
                    if (songs.isNotEmpty()) {
                        ...
                        if (startIndex > -1) {
                            MusicPlayerRemote.openQueueKeepShuffleMode(songs, startIndex, true)
                        } else {
                            Snackbar.make(
    ...
    override fun onMenuItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.action_go_to_start_directory -> {
                setCrumb(
                    BreadCrumbLayout.Crumb(
                        tryGetCanonicalFile(PreferenceUtil.startDirectory)
                    ),
                    true
                )
                return true
    ...
    private fun setCrumb(crumb: BreadCrumbLayout.Crumb?, addToHistory: Boolean) {
        ...
        if (PreferenceUtil.saveLastDirectory) {
            PreferenceUtil.lastDirectory = crumb.file;
        }
