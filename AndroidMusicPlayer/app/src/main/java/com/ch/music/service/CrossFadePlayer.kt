...
class CrossFadePlayer(context: Context) : AudioManagerPlayback(context),
    ...
    private fun crossFade(fadeInMp: MediaPlayer, fadeOutMp: MediaPlayer) {
        isCrossFading = true
        crossFadeAnimator = AudioFader.createFadeAnimator(context, fadeInMp, fadeOutMp) {
    ...
    private fun setDataSourceImpl(
        ...
        try {
            ...
            player.playbackParams =
                PlaybackParams().setSpeed(PreferenceUtil.playbackSpeed).setPitch(PreferenceUtil.playbackPitch)
